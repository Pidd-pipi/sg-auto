...
class QueueWorkerTests(unittest.TestCase):
    ...
    def test_platform_task_opens_chatgpt_with_workspace_and_prompt(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("监控台已预留空任务根目录", query["prompt"][0])
            self.assertIn("--task-root", query["prompt"][0])
    ...
    def test_terminal_status_must_remain_stable(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            workdir = root / "work"
            task_root = workdir / "cy-999-20260919-010000"
            ...
            with mock.patch.object(queue_worker.time, "monotonic", side_effect=lambda: clock[0]), \
                 mock.patch.object(queue_worker.time, "sleep", side_effect=fake_sleep):
                code = queue_worker.wait_for_task(
                    result_file,
                    workdir=workdir,
                    task_root=task_root,
                    args=__import__("argparse").Namespace(
                        task_name="cy-999-20260919-010000",
                        project_code="cy-999",
                        task_type="0-1代码生成",
                        difficulty="困难",
                        side="both",
            ...
            self.assertEqual(writes, ["running", "complete"])
            metadata = json.loads(result_file.read_text(encoding="utf-8"))
            self.assertEqual(metadata["stage"], "task-complete")
